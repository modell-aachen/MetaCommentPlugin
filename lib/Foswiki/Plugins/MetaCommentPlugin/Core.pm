...
sub jsonRpcSaveComment {
    ...
    throw Foswiki::Contrib::JsonRpcContrib::Error( 401, "Access denied" )
      unless Foswiki::Func::checkAccessPermission( 'COMMENT', $wikiName, undef,
        $topic, $web )
    ...
    Foswiki::Func::saveTopic( $web, $topic, $meta, $text,
      ...
      unless DRY;
    writeEvent( "comment",
            "state=(new, unapproved) title="
          . ( $title || '' )
          . ' text='
          . substr( $cmtText, 0, 200 ) ); # SMELL: does not objey approval state
...
sub jsonRpcApproveComment {
    ...
    writeEvent( "commentapprove",
            "state=($comment->{state}) title="
          . ( $comment->{title} || '' )
          . ' text='
...
sub jsonRpcUpdateComment {
    ...
    Foswiki::Func::saveTopic( $web, $topic, $meta, $text,
        { ignorepermissions => 1 } )
      unless DRY;
    writeEvent( "commentupdate",
            "state=($state) title="
          . ( $title || '' )
          . " text="
          . substr( $cmtText, 0, 200 ) );
...
sub isModerator {
    ...
    return 1
      if Foswiki::Func::checkAccessPermission( "MODERATE", $wikiName, undef,
        $topic, $web );
    return 0;
}
...
sub METACOMMENTS {
    ...
    $params->{subformat} =
      Foswiki::Func::expandTemplate( $params->{subtemplate} )
    ...
    $params->{isclosed} =
      ( ( Foswiki::Func::getPreferencesValue("COMMENTSTATE") || 'open' ) eq
          'closed' ) ? 1 : 0;
...
#writeDebug("id=$id, moderation=$params->{moderation}, isModerator=$isModerator, author=$comment->{author}, wikiName=$wikiName, state=$comment->{state}, isclosed=$params->{isclosed}");
            next
              if $params->{author} && $comment->{author} !~ /$params->{author}/;
            ...
            next
              if $params->{state}
                  && (  !$comment->{state}
                      || $comment->{state} !~ /^($params->{state})$/ );
            next
              if $params->{moderation} eq 'on'
                  && !( $isModerator || $comment->{author} eq $wikiName )
                  && (  !$comment->{state}
                      || $comment->{state} !~ /\bapproved\b/ );
            next
              if $params->{moderation} eq 'on'
                  && $params->{isclosed}
                  && (  !$comment->{state}
...
sub formatComments {
    ...
    foreach my $comment (@sortedComments) {
        ...
        my $indexString =
          ( $params->{reverse} eq 'on' ) ? ( $count - $index + 1 ) : $index;
        ...
        if ( $params->{format} =~ /\$subcomments/ && $comment->{children} ) {
            ...
            $subComments = join(
                expandVariables( $params->{separator} ),
                formatComments(
                    $comment->{children}, $params, $indexString, $seen
                )
            );
        ...
        if ( $params->{format} =~ /\$summary/ ) {
            ...
            $summary =~
              s/^\s*\-\-\-\++//g;    # don't remove heading, just strip tml
            $summary =
              $session->renderer->TML2PlainText( $summary, undef, "showvar" )
              . " ...";
            $summary =~ s/\n/<br \/>/g;
        }

        my $permlink =
          Foswiki::Func::getScriptUrl( $comment->{web}, $comment->{topic},
            "view", "#" => "comment" . ( $comment->{name} || 0 ) );
        ...
        my $line = expandVariables(
            ...
            modified =>
              Foswiki::Time::formatTime( ( $comment->{modified} || 0 ) ),
            isodate => Foswiki::Func::formatTime(
...
sub indexTopicHandler {
    ...
    foreach my $comment (@comments) {
        ...
        # set doc fields
        my $createDate =
          Foswiki::Func::formatTime( $comment->{date}, 'iso', 'gmtime' );
        my $date =
          defined( $comment->{modified} )
          ? Foswiki::Func::formatTime( $comment->{modified}, 'iso', 'gmtime' )
          : $createDate;
